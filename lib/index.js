...
class Maker {
  ...
  resources(name, controller, options = {}) {
    ...
    const isOnlyListEmpty = !(Array.isArray(options.only) && options.only.length === 0)
    const isExceptListEmpty = !(Array.isArray(options.except) && options.except.length === 0)
    ...
    this.scope(name, r => {
      ...
      if (typeof controller.patch === 'undefined' && )
    })
  }
}
